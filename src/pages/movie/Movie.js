...
function Movie() {
    ...
    const getData = async () => {
        const response = await apiConfig.get(`/movie/${id}?api_key=1e32e196d77987f5d442a50704d6481e`, { params: { id: id } })
        try {
    ...
    const getVideo = async () => {
        const response = await apiConfig.get(`/movie/${id}/videos?api_key=1e32e196d77987f5d442a50704d6481e`, { params: { id: id } })
        try {
    ...
    const getImages = async () => {
        const response = await apiConfig.get(`/movie/${id}/images?api_key=1e32e196d77987f5d442a50704d6481e`, { params: { id: id } })
        try {
    ...
    const getSimilarMovie = async () => {
        const response = await apiConfig.get(`/movie/${id}/similar?api_key=1e32e196d77987f5d442a50704d6481e`, { params: { id: id } })
        try {
            ...
            return images.backdrops.map((item) => {
                return (
                    <div className="gallery-photo col-6 col-md-4">
                        <div className="m-1">
                            <img src={`${galleryServerUrl}${item.file_path}`} key={item.file_path} alt="movie-poster" />
                        </div>
                    </div>
                )
            })
    ...
    return (
        <Style>
            ...
            <div className="movie-bg" style={{ backgroundImage: `url(${galleryServerUrl}${data.backdrop_path})` }}>
                    ...
                    <div className="container py-5">
                        <div className="row movie py-5 text-white">
                            ...
                            <div className="col-md-9">
                                ...
                                <p>Production Company: <ul>{renderCompany()}</ul></p>
                            </div>
                        </div>
                    </div>
                </div>
