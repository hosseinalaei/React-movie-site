...
const CategoryMovies = () => {
    ...
    useEffect(() => {
        apiConfig.get("/discover/movie?api_key=1e32e196d77987f5d442a50704d6481e", { params: { with_genres: id, page } })
            .then((response) => {
                        ...
                        <div className="movie">
                            <div className="movie-cover">
                                <Link to={`/movie/${item.id}`}>
                                    ...
                                    <h3 className="movie-title">{textDots(item.title || item.original_title)}</h3>
                                </Link>
                            </div>
                        </div>
                    </div>
